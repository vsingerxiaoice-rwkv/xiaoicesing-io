...
class VAEDurationLoss(nn.Module):
    """
    ...
    def __init__(self, offset, loss_type,
                 lambda_pdur=0.6, lambda_wdur=0.3, lambda_sdur=0.1, lambda_KL=0.01):
        super().__init__()
    ...
    def KL_loss(self, mean, log):

        KL = -0.5 * torch.sum(1 + log - mean.pow(2) - log.exp())
        return KL

    def forward(self, dur_pred: Tensor, dur_gt: Tensor, ph2word: Tensor, mean, log) -> Tensor:
        dur_gt = dur_gt.to(dtype=dur_pred.dtype)

        KL = self.KL_loss(mean, log)
        # pdur_loss
        ...
        # combine
        dur_loss = pdur_loss + wdur_loss + sdur_loss + self.lambda_KL * KL

        return dur_loss,self.lambda_KL * KL
