...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
            ...
            layer_scale_init_value: Optional[float] = None, drop_out: float = 0.0, dilation=1

    ):
        super().__init__()
        self.dwconv = nn.Conv1d(dim, dim, kernel_size=7, padding=get_padding(kernel_size=7, dilation=dilation),
    ...
    def __init__(self, dim, lays, time_embed_dim, cond_dim, intermediate_dim=None, layer_scale_init_value=1e-6,
        ...
        self.layers = nn.ModuleList(
            [ConvNeXtBlock(dim=dim,
                           ...
                           layer_scale_init_value=layer_scale_init_value, drop_out=drop_out,
                           dilation=2 ** (i % n_dilates)) for i in range(lays)])
...
class ConvNeXtModel(nn.Module):
    def __init__(self, in_dim, time_embed_dim, cond_dim, dims=None, lays=None, intermediate_dim=None,
        ...
        if lays is None:
            lays = [3, 6, 2, 1]
        if intermediate_dim is None:
            intermediate_dim = [4, 4, 4, 4]
        if n_dilates is None:
            n_dilates = [1, 1, 1, 1]
        dims = [in_dim] + dims
        self.num = len(lays)
